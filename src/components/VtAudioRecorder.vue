...
<template>
    ...
    <div class="vt-audio-recorder">
        ...
        <button class="vt-audio__button" :class="{ 'vt-audio__button--on': isRecording }" :disabled="isRecording"
            @click="starRecord($event)">
            <IconRecord />
        </button>

        <button class="vt-audio__button" :disabled="!isRecording" @click="stopRecord($event)">
            <IconStop />
        </button>

        <button class="vt-audio__button" :title="'Download file' + (hasAudioFile ? (': ' + audioFileName) : '')"
            :disabled="!hasAudioFile" @click="download($event)">
            <IconDownload />
        </button>

        <button class="vt-audio__button" :disabled="isRecording || !hasAudioFile" @click="deleteAudio($event)">
            <IconDelete />
        </button>

        <audio v-show="hasAudioFile" class="vt-audio__player" controls :src="audioUrl" ref="audioElement"></audio>
